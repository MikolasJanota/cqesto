 ...
 */
#ifndef EXPRESSIONS_H_25585
#define EXPRESSIONS_H_25585
...
class Expressions {
public:
  Expressions(const Options& options)
          : options(options) {}
  ...
  inline ID make_and(ID o1, ID o2) {
    if (o1.get_type()==TRUE) return o2;
    if (o2.get_type()==TRUE) return o1;
    if (o1.get_type()==FALSE) return make_false();
    if (o2.get_type()==FALSE) return make_false();
    vector<ID> dv(2);
    dv[0]=o1; dv[1]=o2; return make_and(IDVector(dv));
  }

  inline ID make_or(ID o1, ID o2) { if (o1.get_type()==FALSE) return o2;
    if (o2.get_type()==FALSE) return o1;
    if (o1.get_type()==TRUE) return make_true();
    if (o2.get_type()==TRUE) return make_true();
    vector<ID> dv(2);
    dv[0]=o1; dv[1]=o2; return make_or(IDVector(dv));
  }

  inline Lit open_lit(ID node) const {
    assert(node.get_type()==LITERAL);
    return SATSPC::toLit((int)node.get_index());
  }

  inline bool is_complement_lit(ID o1, ID o2) {
    if (o1.get_type()!=LITERAL) return false;
    if (o2.get_type()!=LITERAL) return false;
  ...
  bool add_operand(vector<ID>& operands, ID o, NodeType top_level) {
    ...
    if (o.get_type()==ide) return true; // NOP
    if (options.get_flatten()&&o.get_type()==top_level) { // flatten
      const IDVector& os = (top_level==OR) ? open_or(o)
                                           : open_and(o) ;
      FOR_EACH(co,os) add_operand(operands, *co, top_level);
      return true;
    } else {
