...
public:
  ...
  virtual R visit(ID expression) {
    ...
    case LITERAL: return visit_lit(expression, SATSPC::toLit((int)expression.get_index()));
    case NEGATION: return visit_not(expression, factory.open_not(expression));
...
class ExpressionArgumentVisitor {
public:
  ...
  virtual R visit(ID expression, A argument) {
    switch (expression.get_type()) {
    case LITERAL: return visit_lit(expression, argument, SATSPC::toLit((int)expression.get_index()));
    case NEGATION: return visit_not(expression, argument, factory.open_not(expression));
    case AND: return visit_and(expression, argument, factory.open_and(expression));
    case OR:  return visit_or(expression, argument, factory.open_or(expression));
...
protected:
   inline const unordered_map<ID,R,ID_hash,ID_equal>& get_m() {
         return m;
   }
private:
...
template <class R, class A>
class MemoizedExpressionArgumentVisitor : public ExpressionArgumentVisitor<R, A> {
public:
  typedef pair<ID,A> KeyType;

  MemoizedExpressionArgumentVisitor(Expressions& factory) :  ExpressionArgumentVisitor<R, A>(factory) { }
...
public:
  ...
  : ExpressionVisitor<std::ostream&>(factory)
  , id_to_orig(NULL)
  , output(output)
  { }

  std::ostream& operator() (ID node) {
    return visit(node);
  }
...
class NiceExpressionPrinter: private ExpressionArgumentVisitor<std::ostream&,size_t> {
public:
  ...
  NiceExpressionPrinter& operator<< (const char c){output<<c;return *this;}
  NiceExpressionPrinter& operator<< (const char* s){output<<s;return *this;}
...
private:
  virtual std::ostream& visit_lit(ID, size_t offset, Lit lit) {
     ...
     if(i==var2name.end()) o(offset)<<"UNKNOWN:"<<var(lit);
  ...
  virtual std::ostream& visit_false(ID, size_t offset) {return o(offset)<<"F";}
  virtual std::ostream& visit_true(ID, size_t offset) {return o(offset)<<"T";}
private:
  std::ostream& output;
  const unordered_map<int,const char*>& var2name;

  std::ostream& o(size_t offset) {
    while (offset) {output<<" "; --offset;}
    return output;
  }
  std::ostream& print_vector(IDVector operands, size_t offset) {
    ...
    FOR_EACH(i,operands) {
      ...
      if (    (t != LITERAL)
           && (t != TRUE)
           && (t != FALSE) ) {nl=true; break;}
    }

    FOR_EACH(i,operands) {
      const ID n=*i;
      if (nl) output<<std::endl;
      if (!first && !nl) output<<" ";
      visit(n, nl ? (offset+1) : 0);
      first = false;
    }
    if (nl) { output<<std::endl; o(offset); }
    return output;
  }
};
...
//  using ExpressionArgumentVisitor<ID,bool>::factory;
 // using MemoizedExpressionArgumentVisitor<bool>::ops;
  ...
  virtual ID visit_and(ID node, bool negate, IDVector operands) {
    return negate ?
      factory.make_or(ops(operands, true, OR))
      : MemoizedExpressionArgumentVisitor<ID,bool>::visit_and(node, false, operands);
  }

  virtual ID visit_or(ID node, bool negate, IDVector operands) {
    return negate ?
      factory.make_and(ops(operands, true, AND))
      : MemoizedExpressionArgumentVisitor<ID,bool>::visit_or(node, false, operands);
  }
