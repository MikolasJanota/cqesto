 ...
 */
#ifndef IMMUTABLEVECTOR_H_23545
#define IMMUTABLEVECTOR_H_23545
...
public:
  ...
  ImmutableVector<T,HashFun>& operator= (const ImmutableVector<T,HashFun>& ls ) {
    decrease();
    _hash_code=ls._hash_code; _size=ls._size; _data=ls._data;
    if (_data!=NULL) ++(_data->count);
    return *this;
  ...
  static ImmutableVector<T,HashFun> conc(const ImmutableVector<T,HashFun>& a,const ImmutableVector<T,HashFun>& b){
    const auto asz=a.size();
    const auto bsz=b.size();
    if(!asz)return b;
    if(!bsz)return a;
    vector<T> rv(asz+bsz);
    for(size_t i=0;i<asz;++i) rv[i]=a[i];
    for(size_t i=0;i<bsz;++i) rv[asz+i]=b[i];
    return ImmutableVector<T,HashFun>(rv);
  }
private:
  struct Data { size_t count; T* elements; };
...
template <class T,class HashFun>
bool ImmutableVector<T,HashFun>::equals(const ImmutableVector<T,HashFun>& other) const {
  if (other._size!=_size) { return false; }
  if (other._data==_data) return true;

  for (size_t i=0; i<_size; ++i)
    if (_data->elements[i]!=other._data->elements[i]) return false;
  return true;
}
...
{
public:
  const_ImmutableVectorIterator(const ImmutableVector<T,HashFun>& ls, size_t x)
  : ls(ls), i(x) { }
  const_ImmutableVectorIterator(const const_ImmutableVectorIterator<T,HashFun>& mit)
  : ls(mit.ls), i(mit.i) { }
  const_ImmutableVectorIterator& operator++()
  {++i; return *this;}
  bool operator==(const const_ImmutableVectorIterator<T,HashFun>& rhs)
  { assert(&ls==&(rhs.ls)); return i==rhs.i;}
  bool operator!=(const const_ImmutableVectorIterator<T,HashFun>& rhs)
  { assert(&ls==&(rhs.ls)); return i!=rhs.i;}
...
ImmutableVector<T,HashFun>::ImmutableVector(const std::vector<T>& es) {
  ...
  for (size_t i=0; i<vsz; ++i) elements[i]=es[i];
  ...
  _size=j;
  _hash_code = 7;
  for (size_t i=0; i<_size; ++i) _hash_code = _hash_code*31 + HashFun()(elements[i]);
}
...
public:
  inline bool operator () (const ImmutableVector<T,HashFun>& v1,const ImmutableVector<T,HashFun>& v2) const
  { return v1.equals(v2); }
};
...
template <class T, class HashFun>
ImmutableVector<T,HashFun> join(ImmutableVector<T,HashFun> a, ImmutableVector<T,HashFun> b) {
  if (a.empty()) return b;
  if (b.empty()) return a;
  vector<T> d(a.size()+b.size());
  for (size_t i=0; i<a.size(); ++i) d[i]=a[i];
  size_t o = a.size();
  for (size_t i=0; i<b.size(); ++i,++o) d[o]=b[i];
  return ImmutableVector<T,HashFun>(d);
}

template <class T, class HashFun>
bool contains(ImmutableVector<T,HashFun> es, const T& e) {
  FOR_EACH(i, es) {if ((*i)==e) return true;}
  return false;
}
}
#endif /* IMMUTABLEVECTOR_H_23545 */
