...
namespace qesto {
   ...
   public:
      ...
      inline size_t qlev(Lit l) const {
         return qlev(var(l));
      }
      ...
      inline size_t qlev_count() const {
         return pref.size();
      }

      inline size_t existential_count() const {
         return ex_count;
      }

      inline size_t universal_count() const {
         return un_count;
      }

      inline Var maxv() const {
         return mxv;
      }
      ...
      inline QuantifierType type(Lit l) const {
         return type(var(l));
      }
      ...
      inline size_t max_qt(QuantifierType qt) const {
         ...
         FOR_EACH(bi, pref) {
            if (bi->first != qt) continue;

            FOR_EACH(vi, bi->second) {
               const auto v = *vi;
               if (v > m) m = v;
            }
         }
   ...
   inline ostream& operator<<(ostream& outs, const LevelInfo& levs) {
      for (size_t ql = 0; ql < levs.qlev_count(); ++ql) {
         ...
         FOR_EACH(vi, levs.level_vars(ql)) outs << " " << *vi;
         outs << "]";
      }
      return outs;
   }
