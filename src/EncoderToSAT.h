#ifndef ENCODERTOSAT_551175
#define ENCODERTOSAT_551175
...
namespace qesto {
   ...
   public:
      EncoderToSAT(Expressions& factory,
                   SATSPC::MiniSatExt& sat_solver, const VariableManager& variable_manager)
      : MemoizedExpressionVisitor<Lit>(factory)
      , sat_solver(sat_solver)
      , variable_manager(variable_manager) {
      }

      Lit operator()(ID node) {
         return visit(node);
      }
   ...
   private:
      ...
      virtual Lit visit_not(ID, ID operand) {
   ...
   private:
      Lit encode_ops(const IDVector& operands, bool and_op) {
         ...
         for (size_t i = 0; i < sz; ++i) {
            ...
            if (and_op) sat_solver.addClause(operand_encoding, ~r);
            else sat_solver.addClause(~operand_encoding, r);
         }
